...
object NotebookDownloader:
  ...
  def downloadAll(
    ...
    for
      ...
      results <- downloadResolved(backend, userToken, allDocs, outputDir)
    yield results

  /** Downloads only the notebooks in `selectedDocs` (a pre-filtered subset of
    * the full document list) and renders them into `outputDir`.
    *
    * The full document list is still required to resolve folder paths correctly.
    *
    * @param backend      sttp backend
    * @param userToken    short-lived user token
    * @param allDocs      the complete list returned by [[RemarkableClient.listAll]]
    * @param selectedDocs the subset of documents to actually download
    * @param outputDir    root output directory
    */
  def downloadSelected(
      backend: RmBackend,
      userToken: UserToken,
      allDocs: List[CloudDocument],
      selectedDocs: List[CloudDocument],
      outputDir: Path,
  ): ZIO[Any, Throwable, List[DownloadResult]] =
    for
      _        <- Console.printLine(s"Downloading ${selectedDocs.length} selected notebook(s)...")
      // Resolve folder paths using the full document list so parent folder names are available.
      resolved  = RemarkableClient.resolveFolderPaths(allDocs)
      // Keep only the resolved entries whose document ID is in the selection.
      selectedIds = selectedDocs.map(_.id).toSet
      toDownload  = resolved.filter(r => selectedIds.contains(r.doc.id))
      results  <- renderResolved(backend, userToken, toDownload, outputDir)
    yield results
  ...
  private def downloadResolved(
      backend: RmBackend,
      userToken: UserToken,
      allDocs: List[CloudDocument],
      outputDir: Path,
  ): ZIO[Any, Throwable, List[DownloadResult]] =
    val resolved = RemarkableClient.resolveFolderPaths(allDocs)
    Console.printLine(s"Resolved ${resolved.length} documents with folder paths") *>
      renderResolved(backend, userToken, resolved, outputDir)
